...
export const signin = async (req, res, next) => {
  ...
  try {
    ...
    const expiryDate = new Date(Date.now() + 1*60*1000*60*24); // 1 hour
    res
      .cookie('access_token', token, { httpOnly: true, expires: expiryDate, domain:'localhost',path: '/',secure:false,sameSite: 'None' })
      .status(200)
      .json(rest);
  } catch (error) {
    next(error);
